...
GridLayout {
    ...
    PrimaryButton {
        ...
        enabled: sessionManager.operationsAllowed && accessManager.canPerform("mission_save") && missionStore.plan.missionState !== "EMPTY"
        opacity: enabled ? 1 : 0.55
        onClicked: missionSyncManager.saveActiveMission()
        ToolTip.visible: hovered && !enabled
        ToolTip.text: !accessManager.canPerform("mission_save") ? "Mission save blocked by local permissions." : (sessionManager.operationsAllowed ? "Create a mission first." : "Device approval is required.")
    }
    PrimaryButton {
        Layout.fillWidth: true
        secondary: true
        enabled: sessionManager.operationsAllowed && accessManager.canPerform("mission_validation") && (root.hasUploadRoute() || missionStore.plan.boundaryOnly)
        ...
        ToolTip.visible: hovered && !enabled
        ToolTip.text: !accessManager.canPerform("mission_validation") ? "Mission validation blocked by local permissions." : (sessionManager.operationsAllowed ? "Add mission geometry before validation." : "Device approval is required.")
    }
    PrimaryButton {
        Layout.fillWidth: true
        enabled: sessionManager.operationsAllowed && accessManager.canPerform("mission_upload") && root.backendReadyForUpload() && vehicleManager.connected && preflightChecklistManager.canUpload && !root.weatherBlocksUpload() && root.hasUploadRoute() && !missionUploadManager.uploading
        ...
        ToolTip.visible: hovered && !enabled
        ToolTip.text: !accessManager.canPerform("mission_upload") ? "Mission upload blocked by local permissions." : root.uploadReason()
    }
    PrimaryButton {
        Layout.fillWidth: true
        enabled: sessionManager.operationsAllowed && accessManager.canPerform("mission_start") && !missionStore.plan.boundaryOnly && vehicleManager.connected && preflightChecklistManager.canStartMission && !missionExecutionManager.executing
        text: missionExecutionManager.executing ? "Executing..." : "Start Mission"
        iconSource: AssetRegistry.icons.boxicons_play
        opacity: enabled ? 1 : 0.55
        onClicked: missionExecutionManager.startMission()
        ToolTip.visible: hovered && !enabled
        ToolTip.text: !accessManager.canPerform("mission_start") ? "Mission start blocked by local permissions." : root.startReason()
    }
}
